...
public class TokenService(IDbContextFactory<DataContext> dbContextFactory) : ITokenService
{
    ...
    public AccessTokenResult GenerateAccessToken(TokenRequest tokenRequest, string? refreshToken)
    {
        try
        {
            ...
            {
                return new AccessTokenResult { StatusCode = (int)HttpStatusCode.BadRequest, Error = "Inavlid request body. Parameters user id and email address must be provided" };
            }
            var claims = new[]
            {
            ...
            if (token == null)
            {
                return new AccessTokenResult { StatusCode = (int)HttpStatusCode.InternalServerError, Error = "Unexpected error occurred while access token was generated" };
            }
    ...
    public async Task<RefreshTokenResult> GenerateRefreshTokenAsync(string userId, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                return new RefreshTokenResult { StatusCode = (int)HttpStatusCode.BadRequest, Error = "Invalid body request. No user id found" };
            }
            var claims = new[]
            {
            ...
            if (token == null)
            {
                return new RefreshTokenResult { StatusCode = (int)HttpStatusCode.InternalServerError, Error = "Unexpected error occurred while token was generated" };
            }

            var cookieOptions = GenerateCookie(DateTimeOffset.Now.AddDays(7));
            if (cookieOptions == null)
            {
                return new RefreshTokenResult { StatusCode = (int)HttpStatusCode.InternalServerError, Error = "Unexpected error occurred while cookie was generated" };
            }

            var result = await SaveRefreshTokenAsync(token, userId, cancellationToken);
            if (!result)
            {
                return new RefreshTokenResult { StatusCode = (int)HttpStatusCode.InternalServerError, Error = "Unexpected error occurred while saving refresh token" };
            }
